...
function Contact(props) {
  ...
  useEffect(() => {
    if(props.showAccordian==="accordian")
    {
      let result = (props.directorActivePosts).find((obj) => {
        return obj.formId === props.directorUpdateFormId;
      });
      let result2 = (props.matchedCandidates).find((obj) => {
        return obj.applicantFormId === props.currentAccordianAppId;
      });
  ...
  const rightSwipe=()=>{
    ...
    let data=  {
      "applicantFormId":props.currentAccordianAppId,
      }
...
const mapStateToProps = (state) => {
  return {
    ...
    showAccordian:state.ScreenIt.showAccordian,
    currentAccordianAppId:state.ScreenIt.currentAccordianAppId,
